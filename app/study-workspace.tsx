...
export default function StudyWorkspace(){
  ...
  return <Tabs value={view} onValueChange={v=>setView(v as View)} className={`app-shell ${view==='coding'?'coding-shell':''}`}><WorkspaceHeader saveStatus={saveStatus} onHome={()=>setView('dashboard')}/><main className="workspace-content">
    {error&&<div className="error-banner" role="alert"><span>{error}</span>{needsSignIn?<a href="/login" target="_top">Sign in with ChatGPT <ArrowUpRight size={15}/></a>:<button onClick={()=>ready?void persistRef.current():void load()}><RefreshCw size={15}/>Retry</button>}</div>}
